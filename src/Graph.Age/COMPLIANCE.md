...
metadata. A before/after inventory of graph labels, indexes, and graph-schema functions proves the
read path creates no store artifacts.
...
The compatibility inventory contains 500 runnable test methods. For this capability set,
`ComplianceInventory.MinimumExecuted(declared)` is 498 methods and the strict compliance guard
passes with 2 expected skips: one for undeclared `ShortestPath` and one for undeclared
`OrderByEntity`. The retained provider run reports 725 runtime cases: 723 passed, 2 capability
