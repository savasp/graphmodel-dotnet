...
internal sealed class AgeValueConverter : IValueConverter
{
    public T? ConvertValue<T>(Property property)
    {
        ...
        if (property.Value is null) return default;

        if (property.Value is SimpleValue simpleValue)
        {
            if (simpleValue.Object is DBNull) return default;
            try { return (T?)AgeSerializationBridge.FromAgeValue(simpleValue.Object, typeof(T)); }
            catch (Exception ex)
            {
