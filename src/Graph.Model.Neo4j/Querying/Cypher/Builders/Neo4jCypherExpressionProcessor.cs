...
internal class Neo4jCypherExpressionProcessor : ICypherExpressionProcessor
{
    ...
    private class SimpleExpressionProcessor
    {
        ...
        private string ProcessMemberExpression(MemberExpression member)
        {
            ...
            {
                if (typeof(IRelationship).IsAssignableFrom(param.Type))
                {
                    if (member.Member.Name == nameof(IRelationship.StartNodeId))
                    {
                        return "src.Id";
                    }

                    if (member.Member.Name == nameof(IRelationship.EndNodeId))
                    {
                        return "tgt.Id";
                    }
                }
