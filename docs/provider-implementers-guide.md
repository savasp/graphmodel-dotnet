...
Simple properties are properties whose types pass `GraphDataModel.IsSimple` or `IsCollectionOfSimple`: supported primitives, enums, string, `Cvoya.Graph.Point`, temporal types, `decimal`, `Guid`, byte arrays, `Uri`, and collections of simple values. Native-sized integers (`IntPtr`/`UIntPtr`, including `nint`/`nuint`) are primitive CLR types but are not graph-storable values. Providers should serialize supported simple properties directly to backend-native property values where possible.
