...
Projections have no property schema to consult, so results materialized through a constructor — records, DTOs, anonymous types — derive the same rules from the constructor parameter's own nullability metadata. A null scalar targeting a non-nullable value-type parameter fails with `GraphException` rather than becoming `default(T)`, and a null element of a single-level collection targeting a non-nullable element type fails with the same indexed diagnostic, naming the parameter instead of a physical property. Nullable parameters and element types preserve nulls and order. Note that the C# compiler emits no nullable-reference metadata for anonymous-type constructor parameters, so their reference elements read as nullable-oblivious and fail closed; project into a named type when a collection legitimately carries null elements.

That guarantee currently covers a wire value that is **present and null**, at one level of collection nesting. A property absent from the record is still satisfied with the parameter's default, which matters for providers that erase stored nulls on write — Neo4j removes a property on `SET n.prop = null`, so its stored nulls read back as absence (#436). Nulls nested inside a collection element, and elements of complex-object collections, are not yet reached (#434). Element nullability also reads as nullable for unconstrained generic type parameters regardless of the type argument (#435).
