...
| Hooks | `eng/agent-hooks/protect-files.sh` (PreToolUse: blocks Edit/Write on `VERSION`, `.github/`, `Directory.Build.props`, `Directory.Packages.props`, `nuget.config`, `.claude/`, `.codex/` — ask the user first; advisory, not a security boundary) |
| Local gate | `.githooks/pre-push` → `eng/ci/ci-local.sh`: fast Release build + `dotnet format` verify, change-scoped to .NET edits (`--full` adds the fast unit lane). Install once per clone with `eng/install-hooks.sh` — linked worktrees inherit it automatically. The full test matrix (unit + Neo4j + AGE) and CodeQL run in CI; a pure branch-deletion push skips the gate. |
