...
- The project layout and what each `src/` and `tests/` project is for: AGENTS.md § "Layout" and § "Build and test". Note that `src/Graph.Model.CompatibilityTests` is the shippable provider compatibility (TCK) suite of abstract contract tests — it executes through a provider test project such as `tests/Graph.Model.Neo4j.Tests`, which needs a running Neo4j.
