...
internal class CypherEngine(GraphContext graphContext)
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var session = query.Transaction?.Session ?? await _driver.AsyncSession();
        var paramBuilder = new CypherParameterBuilder(_entityFactory);
        var resultProcessor = new CypherResultProcessor(_entityFactory, _logger);
        ...
        try
        {
            ...
            {
                cypherParams[key] = paramBuilder.BuildParameterValue(value);
            }

            // Execute query
            var cursor = await session.RunAsync(query.Text, cypherParams);
            var records = await cursor.ToListAsync();

            if (!records.Any())
                return new List<T>();

            // Check if T is a node type with complex properties
            ...
            if (typeof(INode).IsAssignableFrom(targetType))
            {
                ...
                {
                    // This query should have returned paths for complex properties
                    // Group records by the main entity
                    var grouped = records.GroupBy(r =>
                    {
                        // The main node should be aliased as 'n' in our queries
                        if (r.TryGet("n", out var node) && node is INode n)
                            return n.ElementId;
                        return null;
                    })
                    .Where(g => g.Key != null);
                    ...
                    {
                        var entity = resultProcessor.ProcessRecordsWithComplexProperties<T>(group);
                        if (entity != null)
                            results.Add(entity);
                    }
            ...
            // Simple query - process each record independently
            return records
                .Select(r => resultProcessor.ProcessRecord<T>(r))
                .Where(r => r != null)
                .Select(r => r!)
                .ToList();
        }
    ...
    private void PopulateEntitySchema(GraphQueryContext queryContext)
    {
        ...
        try
        {
            queryContext.TargetEntitySchema = graphContext.EntityFactory.GetSchema(entityType);
        }
        catch (Exception ex)
        {
    ...
    private bool ShouldLoadComplexProperties(Type? resultType)
    {
        ...
        var schema = graphContext.EntityFactory.GetSchema(resultType);
        if (schema != null)
        {
            var hasComplexProperties = schema.ComplexProperties.Any();
            _logger?.LogDebug("Type {Type} has complex properties: {HasComplexProperties}",
                resultType.Name, hasComplexProperties);
    ...
    private async Task<T> HandleScalarResultAsync<T>(IResultCursor result, CancellationToken cancellationToken)
    {
        ...
        var convertedValue = SerializationBridge.ConvertFromNeo4jValue(value, typeof(T));
        ...
        CancellationToken cancellationToken)
    {
        ...
        else
        {
            ...
            if (queryContext.TargetEntitySchema?.ComplexProperties.Any() == true)
            {
    ...
    GraphQueryContext queryContext)
    {
        ...
        foreach (var (propName, values) in complexPropsByRelType)
        {
            ...
            if (!schema.ComplexProperties.TryGetValue(neo4jPropName, out var propSchema))
            {
        ...
        GraphQueryContext queryContext)
    {
        ...
        if (record.Values.Count == 2 &&
            queryContext.TargetEntitySchema?.ComplexProperties.Any() == true)
        {
    ...
    private object DeserializeSingleValue(object? value, Type requestedType, GraphQueryContext queryContext)
    {
        ...
        var converted = SerializationBridge.ConvertFromNeo4jValue(value, requestedType);
        ...
        bool useMostDerivedType = true)
    {
        ...
        return graphContext.EntityFactory.Deserialize(targetType, neo4jNode);
    }
        ...
        bool useMostDerivedType = true)
    {
        ...
        return graphContext.EntityFactory.Deserialize(targetType, neo4jRelationship);
    }
}
