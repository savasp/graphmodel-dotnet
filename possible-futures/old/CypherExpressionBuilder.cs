...
{
    // New modular coordinator for handling queries
    private static readonly QueryBuildCoordinator _coordinator = new();
    ...
    public static string BuildCypherExpression(Expression expr, string varName, CypherBuildContext? context = null)
    {
        switch (expr)
        {
            ...
            case MemberExpression me:
                ...
                // Handle relationship property access for WHERE clauses in relationship queries
                if (context?.QueryRootType == GraphQueryContext.QueryRootType.Relationship)
                {
                    // Check if this is accessing a node property through a relationship
                    if (me.Expression is MemberExpression pm &&
                        pm.Expression is ParameterExpression)
                    {
                        var pathProperty = pm.Member.Name; // "Source", "Target", etc.
                        var entityProperty = me.Member.Name; // "FirstName", "Age", etc.

                        // For relationship queries, map to the correct alias
                        return pathProperty switch
                        {
                            "Source" => $"n1.{entityProperty}",     // Source node uses n1
                            "Target" => $"n2.{entityProperty}",     // Target node uses n2
                            _ => $"{varName}.{entityProperty}"      // Relationship properties use current alias
                        };
                    }
                }
                ...
                {
                    // Special handling for Uri property access
                    if (pm.Type == typeof(Uri) && pm.Expression is ParameterExpression)
                    {
                        // This is accessing a property of a Uri (e.g., p.uri.Host)
                        // Since Uri is stored as a string in Neo4j, we need to handle this specially
                        var uriPropertyName = pm.Member.Name; // "uri"
                        var accessedProperty = me.Member.Name; // "Host", "Scheme", etc.

                        return accessedProperty switch
                        {
                            "Host" => $"split(split({varName}.{uriPropertyName}, '://')[1], '/')[0]",
                            "Scheme" => $"split({varName}.{uriPropertyName}, '://')[0]",
                            "Port" => throw new NotSupportedException("Uri.Port property access is not supported in LINQ queries. Consider filtering on client side."),
                            "Path" => throw new NotSupportedException("Uri.Path property access is not supported in LINQ queries. Consider filtering on client side."),
                            "Query" => throw new NotSupportedException("Uri.Query property access is not supported in LINQ queries. Consider filtering on client side."),
                            "Fragment" => throw new NotSupportedException("Uri.Fragment property access is not supported in LINQ queries. Consider filtering on client side."),
                            _ => throw new NotSupportedException($"Uri.{accessedProperty} property access is not supported in LINQ queries. Consider filtering on client side.")
                        };
                    }
            ...
            case BinaryExpression be:
                ...
                if (be.NodeType == ExpressionType.Equal && right == "null")
                {
                    return $"{left} IS NULL";
                }
                if (be.NodeType == ExpressionType.Equal && left == "null")
                {
                    return $"{right} IS NULL";
                }
                if (be.NodeType == ExpressionType.NotEqual && right == "null")
                {
                    return $"{left} IS NOT NULL";
                }
                if (be.NodeType == ExpressionType.NotEqual && left == "null")
                {
    ...
    {
        // Use the new modular coordinator for building queries
        try
        {
            return _coordinator.BuildQuery(expression, elementType, provider, queryContext);
        }
        catch (Exception)
        {
            // Fallback to the original implementation if the new one fails
            // This allows for gradual migration and debugging
            return BuildGraphQueryLegacy(expression, elementType, provider, queryContext);
        }
    }

    // Original implementation as fallback during migration
    private static (string cypher, Dictionary<string, object?> parameters, CypherBuildContext context) BuildGraphQueryLegacy(
            Expression expression,
            Type elementType,
            Neo4jGraphProvider provider,
            GraphQueryContext? queryContext = null)
        ...
        Neo4jGraphProvider provider)
    {
        switch (expression)
        {
            ...
            case ConstantExpression constant when constant.Type.IsGenericType &&
                ...
                {
                    // For node queries (default), existing logic...
        ...
        Neo4jGraphProvider provider)
    {
        ...
        // TODO: We probably need to get the labels for the class hierarchy here
        // and then create the right strings further down.
    ...
    private string BuildCypherQuery(CypherBuildContext context)
    {
            ...
            if (context.RootType != null)
            {
                ...
                if (context.QueryRootType == GraphQueryContext.QueryRootType.Relationship)
                {
                    ...
                    else
                    {
                        ...
                        // Store the labels for WHERE clause
                        context.InheritanceLabels = labels;
                    }

                    // Set the current alias to the relationship for proper WHERE clause handling
                    context.CurrentAlias = "r1";
                }
                else
                {
        ...
        // Handle node inheritance labels for relationship queries with node filters
        if (context.NodeInheritanceLabels != null)
        {
            foreach (var (alias, labels) in context.NodeInheritanceLabels)
            {
                if (labels.Count > 1)
                {
                    var labelConditions = labels.Select(l => $"{alias}:{l}");
                    whereConditions.Add($"({string.Join(" OR ", labelConditions)})");
                }
            }
        }

        // Handle relationship inheritance labels only when:
        // 1. We don't have node inheritance labels (no node filters)
        // 2. AND we actually have relationship inheritance labels to apply
        if (context.InheritanceLabels != null && context.InheritanceLabels.Count > 1 &&
            (context.NodeInheritanceLabels == null || context.NodeInheritanceLabels.Count == 0))
        {
            if (context.QueryRootType == GraphQueryContext.QueryRootType.Relationship)
            {
                // For relationships, use type() function
                var typeConditions = context.InheritanceLabels.Select(l => $"type({context.CurrentAlias}) = '{l}'");
                whereConditions.Add($"({string.Join(" OR ", typeConditions)})");
            }
            else
            {
                // For nodes, use label syntax
    ...
    // Update ProcessWhereClause to track node filters
    private void ProcessWhereClause(Expression expression, CypherBuildContext context)
    {
        // Check if this WHERE clause contains node property filters
        if (context.QueryRootType == GraphQueryContext.QueryRootType.Relationship)
        {
            context.HasNodeFilters = ContainsNodePropertyAccess(expression);

            // If we have node filters, we need to determine the node type and its labels
            if (context.HasNodeFilters)
            {
                var nodeType = ExtractNodeTypeFromExpression(expression);
                if (nodeType != null)
                {
                    var nodeLabels = Neo4jTypeManager.GetLabelsForAssignableTypes(nodeType).ToList();
                    if (nodeLabels.Count > 1)
                    {
                        context.NodeInheritanceLabels ??= new Dictionary<string, List<string>>();

                        // Determine which node alias to use based on the expression
                        var nodeAlias = DetermineNodeAlias(expression);
                        context.NodeInheritanceLabels[nodeAlias] = nodeLabels;
                    }
                }
            }
        }
    ...
    // Helper to determine which node alias to use
    private string DetermineNodeAlias(Expression expression)
    {
        // Check if the expression references Source or Target
        var visitor = new NodeAliasVisitor();
        visitor.Visit(expression);

        if (visitor.ReferencesSource)
            return "n1"; // Source node alias
        else if (visitor.ReferencesTarget)
            return "n2"; // Target node alias
        else
            return "n"; // Default node alias
    }

    // Visitor class to determine which node is being referenced
    private class NodeAliasVisitor : ExpressionVisitor
    {
        public bool ReferencesSource { get; private set; }
        ...
        protected override Expression VisitMember(MemberExpression node)
        {
            ...
            {
                if (parent.Member.Name == "Source")
                    ReferencesSource = true;
                else if (parent.Member.Name == "Target")
    ...
    // Helper method to check if a property belongs to a node type
    private bool IsNodeProperty(PropertyInfo? property)
    {
        if (property == null) return false;

        // Check if the declaring type implements INode
        return property.DeclaringType != null &&
               typeof(INode).IsAssignableFrom(property.DeclaringType);
    }

    // Add helper method to check if expression contains node property access
    private bool ContainsNodePropertyAccess(Expression expression)
    {
        switch (expression)
        {
            case MemberExpression me:
                // Check if this is accessing a property through Source or Target
                if (me.Expression is MemberExpression parentMe)
                {
                    if (parentMe.Member.Name == "Source" || parentMe.Member.Name == "Target")
                    {
                        // This is accessing a node property through a relationship navigation
                        return true;
                    }
                }

                if (me.Expression is ParameterExpression && IsNodeProperty(me.Member as PropertyInfo))
                    return true;

                if (me.Expression != null)
                    return ContainsNodePropertyAccess(me.Expression);

                return false;

            case BinaryExpression be:
                return ContainsNodePropertyAccess(be.Left) || ContainsNodePropertyAccess(be.Right);

            case UnaryExpression ue:
                return ContainsNodePropertyAccess(ue.Operand);

            case MethodCallExpression mce:
                // Check method arguments
                foreach (var arg in mce.Arguments)
                {
                    if (ContainsNodePropertyAccess(arg)) return true;
                }
                if (mce.Object != null)
                    return ContainsNodePropertyAccess(mce.Object);
                return false;
    ...
    // Add helper to extract node type from filter expression
    private Type? ExtractNodeTypeFromExpression(Expression expression)
    {
        switch (expression)
        {
            case MemberExpression me:
                // Handle navigation properties like r.Source.FirstName
                if (me.Expression is MemberExpression parentMe)
                {
                    if (parentMe.Member.Name == "Source" && parentMe.Member is PropertyInfo sourceProp)
                    {
                        return sourceProp.PropertyType; // Return the Source node type
                    }
                    else if (parentMe.Member.Name == "Target" && parentMe.Member is PropertyInfo targetProp)
                    {
                        return targetProp.PropertyType; // Return the Target node type
                    }
                }

                if (me.Member is PropertyInfo prop && IsNodeProperty(prop))
                    return prop.DeclaringType;

                if (me.Expression != null)
                    return ExtractNodeTypeFromExpression(me.Expression);

                return null;

            case BinaryExpression be:
                return ExtractNodeTypeFromExpression(be.Left) ?? ExtractNodeTypeFromExpression(be.Right);

            case MethodCallExpression mce:
                // Check method arguments
                foreach (var arg in mce.Arguments)
                {
                    var type = ExtractNodeTypeFromExpression(arg);
                    if (type != null) return type;
                }
                if (mce.Object != null)
                    return ExtractNodeTypeFromExpression(mce.Object);
                return null;
    ...
    private string BuildCypherExpressionWithVariableMapping(Expression expression, string fromParam, string toVar, CypherBuildContext context)
    {
        switch (expression)
        {
            ...
            case BinaryExpression binaryExpr:
                ...
                // Handle null comparisons specially
                if (binaryExpr.NodeType == ExpressionType.Equal && rightExpr == "null")
                {
                    return $"{leftExpr} IS NULL";
                }
                if (binaryExpr.NodeType == ExpressionType.Equal && leftExpr == "null")
                {
                    return $"{rightExpr} IS NULL";
                }
                if (binaryExpr.NodeType == ExpressionType.NotEqual && rightExpr == "null")
                {
                    return $"{leftExpr} IS NOT NULL";
                }
                if (binaryExpr.NodeType == ExpressionType.NotEqual && leftExpr == "null")
                {
                    return $"{rightExpr} IS NOT NULL";
                }
    ...
    private static object? ConvertValue(object? value, Type targetType)
    {
        ...
        if (targetType == typeof(Uri) && value is string uriString)
        {
