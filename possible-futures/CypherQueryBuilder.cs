...
internal class CypherQueryBuilder(CypherQueryContext context)
{
    ...
    private void AppendComplexPropertyMatchesForPathSegment(StringBuilder query)
    {
        ...
        // For multihop queries, we need to unwind the path into individual segments
        if (HasDepthConstraints && (_maxDepth > 1 || _minDepth > 1))
        {
            AppendMultihopPathSegmentQuery(query, src, rel, tgt);
            return;
        }

        // Single hop logic remains the same
        AppendSingleHopPathSegmentQuery(query, src, rel, tgt);
    }

    private void AppendMultihopPathSegmentQuery(StringBuilder query, string src, string rel, string tgt)
    {
        _logger.LogDebug("Building multihop path segment query for unwinding");

        // Much simpler approach - just unwind the relationship list directly
        query.AppendLine($@"
        // Unwind the relationship list into individual relationships
        UNWIND {rel} AS segment_rel
        WITH segment_rel,
             startNode(segment_rel) AS segment_src,
             endNode(segment_rel) AS segment_tgt");
        ...
        {
            PathSegmentProjectionEnum.StartNode => (true, false),
            PathSegmentProjectionEnum.EndNode => (false, true),
            PathSegmentProjectionEnum.Relationship => (true, true),
            PathSegmentProjectionEnum.Full => (true, true),
            _ => (false, false)
        };

        var currentWith = "segment_src, segment_rel, segment_tgt";

        if (needsSourceProps)
        {
            query.AppendLine($@"
        // Complex properties from segment source node
        OPTIONAL MATCH src_path = (segment_src)-[rels*1..]->(prop)
        WHERE ALL(rel in rels WHERE type(rel) STARTS WITH '{GraphDataModel.PropertyRelationshipTypeNamePrefix}')
        WITH {currentWith},
            CASE 
                WHEN src_path IS NULL THEN []
                ELSE [j IN range(0, size(rels)-1) | {{
                    ParentNode: CASE 
                        WHEN j = 0 THEN segment_src
                        ELSE nodes(src_path)[j]
                    END,
                    Relationship: rels[j],
                    Property: nodes(src_path)[j+1]
                }}]
            END AS src_flat_property
        WITH {currentWith},
            reduce(flat = [], l IN collect(src_flat_property) | flat + l) AS src_flat_properties
        ...
        {
            query.AppendLine($@"
        // Complex properties from segment target node
        OPTIONAL MATCH tgt_path = (segment_tgt)-[trels*1..]->(tprop)
        ...
        WITH {currentWith},
            CASE 
                ...
                ELSE [j IN range(0, size(trels)-1) | {{
                    ParentNode: CASE 
                        WHEN j = 0 THEN segment_tgt
                        ELSE nodes(tgt_path)[j]
                    END,
                    Relationship: trels[j],
                    Property: nodes(tgt_path)[j+1]
                }}]
        ...
        // Build the return clause for individual segments
        var returnClause = PathSegmentProjection switch
        {
            PathSegmentProjectionEnum.EndNode => needsTargetProps
                ? @"RETURN {
            Node: segment_tgt,
            ...
            PathSegmentProjectionEnum.StartNode => needsSourceProps
                ? @"RETURN {
            Node: segment_src,
            ComplexProperties: src_flat_properties
        } AS Node"
            ...
            PathSegmentProjectionEnum.Relationship => (needsSourceProps, needsTargetProps) switch
            {
                (true, true) => @"RETURN {
            Relationship: segment_rel,
            StartNode: {
                Node: segment_src,
                ComplexProperties: src_flat_properties
            },
            ...
            PathSegmentProjectionEnum.Full => (needsSourceProps, needsTargetProps) switch
            {
                (true, true) => @"RETURN {
            StartNode: {
                Node: segment_src,
    ...
    private void AppendSingleHopPathSegmentQuery(StringBuilder query, string src, string rel, string tgt)
    {
        _logger.LogDebug("Building single hop path segment query");

        // Existing single-hop logic remains unchanged
        var (needsSourceProps, needsTargetProps) = PathSegmentProjection switch
        {
            PathSegmentProjectionEnum.StartNode => (true, false),
            PathSegmentProjectionEnum.EndNode => (false, true),
    ...
    private void BuildPendingPathSegmentPattern()
    {
        ...
        // For multihop queries, we need to capture the path variable
        string pattern = HasDepthConstraints && (_maxDepth > 1 || _minDepth > 1)
            ? direction switch
            {
                GraphTraversalDirection.Outgoing => $"path = ({p.SourceAlias}:{sourceLabel})-[{p.RelAlias}:{relLabel}*{GetDepthPattern()}]->({p.TargetAlias}:{targetLabel})",
                GraphTraversalDirection.Incoming => $"path = ({p.SourceAlias}:{sourceLabel})<-[{p.RelAlias}:{relLabel}*{GetDepthPattern()}]-({p.TargetAlias}:{targetLabel})",
                GraphTraversalDirection.Both => $"path = ({p.SourceAlias}:{sourceLabel})-[{p.RelAlias}:{relLabel}*{GetDepthPattern()}]-({p.TargetAlias}:{targetLabel})",
                _ => throw new NotSupportedException($"Unknown traversal direction: {direction}")
            }
            : direction switch
            {
                GraphTraversalDirection.Outgoing => $"({p.SourceAlias}:{sourceLabel})-[{p.RelAlias}:{relLabel}]->({p.TargetAlias}:{targetLabel})",
                GraphTraversalDirection.Incoming => $"({p.SourceAlias}:{sourceLabel})<-[{p.RelAlias}:{relLabel}]-({p.TargetAlias}:{targetLabel})",
                GraphTraversalDirection.Both => $"({p.SourceAlias}:{sourceLabel})-[{p.RelAlias}:{relLabel}]-({p.TargetAlias}:{targetLabel})",
                _ => throw new NotSupportedException($"Unknown traversal direction: {direction}")
            };
