...
public class CG005_InvalidPropertyTypeForRelationshipTests
{
    ...
    public async Task RelationshipWithSystemDrawingPointType_ProducesDiagnostics()
    {
        ...
        var expected = new[]
        {
            VerifyCS.Diagnostic("CG005").WithLocation(0).WithArguments("Location", "TestRelationship", "Point"),
            VerifyCS.Diagnostic("CG005").WithLocation(1).WithArguments("MaybeLocation", "TestRelationship", "Nullable<Point>"),
            VerifyCS.Diagnostic("CG005").WithLocation(2).WithArguments("Locations", "TestRelationship", "List<Point>"),
        };
