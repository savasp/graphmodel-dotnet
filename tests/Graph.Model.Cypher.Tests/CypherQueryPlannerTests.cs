...
using Cvoya.Graph.Model.Cypher.Planning;
using Cvoya.Graph.Model.Cypher.Validation;
using Cvoya.Graph.Model.Querying;
using AstBinaryExpression = Cvoya.Graph.Model.Cypher.Ast.Expressions.BinaryExpression;
using AstUnaryExpression = Cvoya.Graph.Model.Cypher.Ast.Expressions.UnaryExpression;
...
public class CypherQueryPlannerTests
{
    ...
    public void Plan_LowersTypedTraversalAsRelationshipPattern()
    {
        var traversal = new TraversalStep(
            ...
            new Cvoya.Graph.Model.Querying.DepthRange(1, 3),
            [],
            typeof(Person),
            typeof(Knows));
        ...
        Assert.Equal(new Cvoya.Graph.Model.Cypher.Ast.DepthRange(1, 3), relationship.Depth);
        new CypherAstValidator().Run(statement);
    }
