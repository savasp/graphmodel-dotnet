...
public sealed class LegacyRelationshipMetadataTests(Neo4jHarness harness) : Neo4jTest(harness)
{
    [Fact]
        ...
        CancellationToken cancellationToken)
    {
        ...
        var result = await neo4jTransaction.Transaction.RunAsync(
            ...
            RETURN r.Id AS id,
                   ...
                   head(r.Labels) AS canonicalLabel,
        ...
        foreach (var identity in expected)
        {
            ...
            Assert.Equal(identity.StorageType, record["physicalType"].As<string>());
