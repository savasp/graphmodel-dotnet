...
public sealed class TestInfrastructureFixture : IAsyncLifetime
{
    ...
    public async Task<AgeGraphStore> GetGraphAsync(bool getNewGraph = false)
    {
        if (getNewGraph)
        {
            ...
            var freshStore = new AgeGraphStore(connectionString, freshGraphName, schemaRegistry, loggerFactory);
        ...
        var pooledStore = new AgeGraphStore(connectionString, graphName, schemaRegistry, loggerFactory);
